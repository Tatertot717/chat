...
int main() {
	...
	struct sockaddr_in addr = {.sin_family = AF_INET,
							   ...
							   .sin_addr.s_addr = INADDR_ANY}; // create our ipv4 context
	...
	while (!shutdown_requested) { // begin main loop
		...
		if (accept_incoming_connections(ssl_ctx,
										server_fd)) // accept any incoming requests, and skip queueing for this loop
			continue;
...
void db_client_inactive(
		const char *username) { // mark a user as inactive in the database. Called when disconnecting a client
	sqlite3_stmt *stmt;
...
void db_client_active(const char *username) { // does the opposite, when a client logs in. When they are created, they
											  // are created active.
	sqlite3_stmt *stmt;
...
int shutdown_ssl_gracefully(SSL *ssl, int fd,
							int timeout_ms) { // attempts to shut down a ssl/tcp connection without closing violently
	...
	while (1) {
		...
		int err = SSL_get_error(
				ssl, ret); // else error, probably improper shutdown on other side or client has stopped responding
		if (err == SSL_ERROR_WANT_READ ||
			err == SSL_ERROR_WANT_WRITE) { // if error is recoverable, wait for client to respond
...
void remove_client(int i) { // delete a client, and clear related fields
	...
	if (i < 0 || i >= client_count || !clients[i].active) { // sometimes this method can be called multiple times on
															// same client, and this makes sure nothing breaks
...
void disconnect_client(Client *client, const char *reason) { // wrapper for remove client
	...
	pthread_mutex_unlock(&client_mutex); // this could be dangerous here if another thread is held before shifting
										 // array, but we ball.
...
void display_clients(const Client *client) { // send the client a basic json list of active clients and let them parse
											 // it
...
int make_non_blocking(int fd) { // small helper to make the sslread (or on a lower level, recv) return -1 immediately
								// with EAGAIN if no data
...
void *worker_thread() { // main worker entry point
	while (!shutdown_requested) { // main worker loop
		...
		if (client->state == INITIAL || client->state == CREATE_USERNAME || client->state == CREATE_PASSWORD ||
			...
			if (client->buffer[0] ==
				'b') { // check client for all server state codes, but its first letter of each message
				...
				if (target) { // if found
					...
					SSL_write(client->ssl, "wConnection request sent\n", 26);
				} else {
					SSL_write(client->ssl, "aUser not available.\n", 22);
				}
				pthread_mutex_unlock(&client_mutex);
			...
			if (client->buffer[0] == 'h') { // if message was not for handoff, ignore
				if (client->buffer[1] == 'y' || client->buffer[1] == 'Y') { // if accepted
					...
					if (partner && partner->active) { // sanity check
						char buf[128];
						snprintf(buf, sizeof(buf), "hConnect to %s:%d:%s\n", inet_ntoa(client->address.sin_addr), 4400,
								 client->username);
						SSL_write(partner->ssl, buf, (int) strlen(buf));

						snprintf(buf, sizeof(buf), "hConnect to %s:%d:%s\n", inet_ntoa(partner->address.sin_addr), 4400,
								 partner->username);
...
bool accept_incoming_connections(SSL_CTX *ssl_ctx, const int server_fd) {
	if (fds[0].revents & POLLIN) { // if activity on server fd
		...
		const int client_fd =
				accept(server_fd, (struct sockaddr *) &cli_addr, &cli_len); // accept it and throw data into cli_addr
...
int hash_and_salt(char *password, const char *salt) { // hash and salt
	...
	if (PKCS5_PBKDF2_HMAC(password, strlen(password), (const unsigned char *) salt, 16, iterations, EVP_sha256(),
						  keylen,
						  (unsigned char *) hash) != 1) { // already have ssl, and this is included in the library
...
void broadcast_message(Client *sender, char *msg_body) { // wrapper for sending
	strcat(msg_body, "\n");
